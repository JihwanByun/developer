 ...
 */
public class 게리맨더링_연습 {
    static int N, All_p, min_Cnt, a_population,b_population;
    ...
    private static boolean divide(boolean[] visited) {
        ...
        bfs(A,a_population, copy); // A 연결된 노드들 인구 구하기
        ...
        bfs(B, b_population, copy);
        System.out.println(a_population + " " + b_population);

        if (population[0] + population[1] == All_p) return true;

        return false;
    }
    private static void bfs(List<Integer> list, int population, boolean[] copy) { //값 복사
        boolean[] visited_Q = new boolean[N + 1];

        Queue<Integer> q = new LinkedList<>();
        q.add(list.get(0));
        visited_Q[list.get(0)]= true; //처음 가는 곳 방문 체크
        population += cnt[list.get(0)]; // A나 B에 인구수 넣기
